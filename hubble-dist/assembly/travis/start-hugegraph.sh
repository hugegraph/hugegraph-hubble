...
CONF=${TOP}/conf
PID_FILE=${BIN}/pid
LOG_FILE=${TOP}/logs/hugegraph-server.log
...
echo -n "Starting HugeGraphServer..."
if [[ -n "$VERBOSE" ]]; then
...
wait_for_startup ${PID} 'HugeGraphServer' ${LOG_FILE} ${SERVER_STARTUP_TIMEOUT_S} || {
    echo "See ${LOG_FILE} for HugeGraphServer log output." >&2
    exit 1
}
disown
